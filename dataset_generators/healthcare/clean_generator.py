...
def generate_dim_patient(n: int = 5000, seed: int | None = None) -> pd.DataFrame:
    """
...
def generate_dim_provider(n: int = 500, seed: int | None = None) -> pd.DataFrame:
    """
...
def generate_dim_date(start: str = "2022-01-01", end: str = "2025-12-31", seed: int | None = None) -> pd.DataFrame:
    """
...
def generate_dim_diagnosis(seed: int | None = None) -> pd.DataFrame:
    """
...
def generate_dim_procedure(seed: int | None = None) -> pd.DataFrame:
    """
...
def generate_dim_payer(seed: int | None = None) -> pd.DataFrame:
    """
...
def generate_fact_encounters(
    ...
    diagnosis_lookup = dim_diagnosis.set_index("diagnosis_key")[["icd10_code", "description", "category"]].to_dict(
        "index"
    )
...
def generate_fact_claims(
    fact_encounters: pd.DataFrame, dim_payer: pd.DataFrame, n: int = 60000, seed: int | None = None
) -> pd.DataFrame:
    """
...
def generate_healthcare_star_schema(
    ...
    fact_encounters = generate_fact_encounters(
        dim_patient, dim_provider, dim_date, dim_diagnosis, n=n_encounters, seed=seed
    )

    print(f"  - fact_claims (~{n_claims:,} rows)")
    fact_claims = generate_fact_claims(fact_encounters, dim_payer, n=n_claims, seed=seed)

    print(f"  - fact_prescriptions ({n_prescriptions:,} rows)")
    fact_prescriptions = generate_fact_prescriptions(
        fact_encounters, dim_patient, dim_provider, n=n_prescriptions, seed=seed
    )
