...
class TestQueryGenerator:
    ...
    def _register_naming_queries(self) -> None:
        ...
        revenue_base_queries = [
            ("revenue_ambiguity_1", "What was total revenue last quarter?",
             "SUM from net_amount column with date filtering"),
            ("revenue_ambiguity_2", "Show me our monthly revenue for 2024",
             "GROUP BY month with net_amount aggregation"),
            ("revenue_ambiguity_3", "What's our revenue by product category?",
             "JOIN to dim_product and GROUP BY category"),
            ("revenue_ambiguity_4", "Compare Q1 vs Q2 revenue",
             "fiscal_quarter grouping with net_amount"),
            ("revenue_ambiguity_5", "What was revenue for the Enterprise segment?",
             "JOIN to dim_customer with segment filter"),
            ("revenue_ambiguity_6", "Show year-over-year revenue growth",
             "Compare current and prior year using date dimension"),
            ("revenue_ambiguity_7", "What's the average revenue per customer?",
             "SUM(net_amount) / COUNT(DISTINCT customer_key)"),
        ]

        for qid, question, note in revenue_base_queries:
            self._queries.append(TestQuery(
                id=qid,
...
""".strip(),
    ...
    def get_by_id(self, query_id: str) -> Optional[TestQuery]:
        """
    ...
    def get_for_patterns(self, pattern_ids: List[str]) -> List[TestQuery]:
        ...
        return [
            ...
            if pattern_set.intersection(set(q.related_patterns))
        ]
    ...
    def get_for_traps(self, trap_ids: List[str]) -> List[TestQuery]:
        ...
        return [
            q for q in self._queries
            if trap_set.intersection(set(q.related_traps))
        ]
    ...
    def _generate_markdown_script(
        ...
        for cat in category_order:
            ...
            for q in categories[cat]:
                ...
                lines.append("")
                lines.append(f"**Expected Failure:**")
...
_generator: Optional[TestQueryGenerator] = None
