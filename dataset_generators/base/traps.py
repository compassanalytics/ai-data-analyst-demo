...
class TrapRevenue(TrapColumn):
    ...
    source_columns: list[str] = field(
        default_factory=lambda: ["quantity_sold", "qty", "quantity", "QTY_SOLD", "units_sold", "unit_sold"]
    )
    ...
    def generate(self, df: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(np.random.randint(1, 50, size=len(df)), index=df.index, name=self.column_name)
...
class TrapTotal(TrapColumn):
    ...
    # Source columns to look for (in priority order)
    source_columns: list[str] = field(
        default_factory=lambda: ["gross_amount", "gross_amt", "gross_sales", "GROSS", "gross_revenue"]
    )
    ...
    def generate(self, df: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(np.random.uniform(10, 500, size=len(df)).round(2), index=df.index, name=self.column_name)
...
class TrapDate(TrapColumn):
    ...
    def generate(self, df: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(np.random.randint(start_epoch, end_epoch, size=len(df)), index=df.index, name=self.column_name)
...
class TrapStatus(TrapColumn):
    ...
    # Source columns to look for (in priority order)
    source_columns: list[str] = field(default_factory=lambda: ["is_active", "active", "is_current", "status"])
...
class TrapMargin(TrapColumn):
    ...
    def can_apply(self, df: pd.DataFrame) -> bool:
        ...
        has_amounts = ("net_amount" in df.columns or "net_amt" in df.columns) and (
            "cost_amount" in df.columns or "cost_amt" in df.columns
        )
        has_prices = ("unit_price" in df.columns or "px" in df.columns) and ("unit_cost" in df.columns)
    ...
    def generate(self, df: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(np.random.uniform(15, 45, size=len(df)).round(2), index=df.index, name=self.column_name)
...
class TrapCustomerCount(TrapColumn):
    ...
    source_columns: list[str] = field(default_factory=lambda: ["sale_key", "txn_id", "transaction_id", "sale_id"])
    ...
    def generate(self, df: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(range(1, len(df) + 1), index=df.index, name=self.column_name)
...
class TrapDiscount(TrapColumn):
    ...
    # Source columns to look for (in priority order)
    source_columns: list[str] = field(default_factory=lambda: ["discount_amount", "disc_amt", "DISC_$", "disc_$"])
...
class TrapRegistry:
    ...
    def get(self, trap_id: str) -> TrapColumn:
        ...
        if trap_id not in self._traps:
            raise KeyError(f"Trap '{trap_id}' not found. Available: {list(self._traps.keys())}")
    ...
    def get_active_traps(self, cleanliness: int) -> list[TrapColumn]:
        ...
        """
        return [trap for trap in self._traps.values() if cleanliness < trap.apply_at_cleanliness]

    def apply_traps(
        self,
        df: pd.DataFrame,
        cleanliness: int,
        include_traps: list[str] | None = None,
        exclude_traps: list[str] | None = None,
    ) -> pd.DataFrame:
        """
        ...
        if include_traps is not None:
            ...
            traps_to_apply = [
                self._traps[trap_id]
                for trap_id in include_traps
                if trap_id in self._traps and trap_id not in exclude_traps
            ]
        else:
            # Use cleanliness-based selection
            traps_to_apply = [trap for trap in self.get_active_traps(cleanliness) if trap.id not in exclude_traps]
...
_registry: TrapRegistry | None = None
